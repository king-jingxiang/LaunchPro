...
LaunchPro 是一款快速、简洁的桌面应用，帮助开发者在一个地方统一管理所有本地项目。快速浏览、整理并用你喜欢的 IDE 或编辑器打开任意项目 —— 界面干净，体验原生。
...
LaunchPro 基于 [Tauri v2](https://tauri.app) 构建，为所有主流桌面平台提供真正的原生二进制包：
...
1. 前往 [Releases](https://github.com/king-jingxiang/LaunchPro/releases) 页面
2. 根据你的平台下载对应安装包：
   - **macOS**：`LaunchPro_x.x.x_aarch64.dmg`（Apple Silicon）或 `LaunchPro_x.x.x_x64.dmg`（Intel）
   - **Windows**：`LaunchPro_x.x.x_x64-setup.exe` 或 `LaunchPro_x.x.x_x64_en-US.msi`
   - **Linux**：`LaunchPro_x.x.x_amd64.deb` 或 `LaunchPro_x.x.x_amd64.AppImage`
3. 安装并运行
